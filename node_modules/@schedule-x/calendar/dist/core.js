...
const viewWeek = createPreactView(config$5);
const createViewWeek = () => createPreactView(config$5);
...
const viewDay = createPreactView(config$4);
const createViewDay = () => createPreactView(config$4);
...
const viewMonthGrid = createPreactView(config$3);
const createViewMonthGrid = () => createPreactView(config$3);
...
function MonthAgendaDay({ day, isActive, setActiveDate, isLeadingOrTrailing, }) {
    ...
    const monthAgendaDateDotsCCID = useState(monthAgendaDateDotsCustomComponent ? randomStringId() : '')[0];
    useEffect(() => {
...
function MonthAgendaEvents({ events, customComponentKey, customContentKey, }) {
    const $app = useContext(AppContext);
    return (jsx("div", { className: "sx__month-agenda-events", children: events.length ? (events.map((event) => (jsx(MonthAgendaEvent, { calendarEvent: event, customComponentKey: customComponentKey, customContentKey: customContentKey }, event.id)))) : (jsx("div", { className: "sx__month-agenda-events__empty", children: $app.translate('No events') })) }));
}
...
const MonthAgendaWrapper = ({ $app, id }) => {
    ...
    return (jsx(AppContext.Provider, { value: $app, children: jsxs("div", { id: id, className: "sx__month-agenda-wrapper", children: [jsx(MonthAgendaDayNames, { week: agenda.weeks[0] }), jsx("div", { className: "sx__month-agenda-weeks", children: agenda.weeks.map((week, index) => (jsx(MonthAgendaWeek, { week: week, setActiveDate: (date) => ($app.datePickerState.selectedDate.value = date), activeDate: $app.datePickerState.selectedDate.value, isLeadingOrTrailing: (date) => date.month !== selectedMonth }, index))) }), jsx(MonthAgendaEvents, { events: ((_a = agenda.weeks
                        .flat()
...
const viewMonthAgenda = createPreactView(config$2);
const createViewMonthAgenda = () => createPreactView(config$2);
...
const WeekAgendaWrapper = ({ $app, id }) => {
    ...
    return (jsx(AppContext.Provider, { value: $app, children: jsxs("div", { id: id, className: "sx__month-agenda-wrapper sx__week-agenda-wrapper", children: [jsxs("div", { className: "sx__week-agenda-header", children: [jsx(MonthAgendaDayNames, { week: agenda.weeks[0] }), jsx("div", { className: "sx__month-agenda-weeks", children: agenda.weeks.map((week, index) => (jsx(MonthAgendaWeek, { week: week, setActiveDate: (date) => ($app.datePickerState.selectedDate.value = date), activeDate: $app.datePickerState.selectedDate.value }, index))) })] }), jsx(MonthAgendaEvents, { events: ((_a = agenda.weeks
                        .flat()
                        .find((day) => isSameDay(day.date, $app.datePickerState.selectedDate.value))) === null || _a === void 0 ? void 0 : _a.events) || [], customComponentKey: "weekAgendaEvent", customContentKey: "weekAgenda" }, $app.datePickerState.selectedDate.value)] }) }));
};
...
const viewWeekAgenda = createPreactView(config$1);
const createViewWeekAgenda = () => createPreactView(config$1);
...
export { AppContext, CalendarApp, DateGridDay, TimeAxis, TimeGridDay, createCalendar, createPreactView, createViewDay, createViewList, createViewMonthAgenda, createViewMonthGrid, createViewWeek, createViewWeekAgenda, externalEventToInternal, setRangeForDay, setRangeForMonth, setRangeForWeek, sortEventsForWeekView, toDateString$1 as toDateString, toDateTimeString, toJSDate, toTimeString, viewDay, viewList, viewMonthAgenda, viewMonthGrid, viewWeek, viewWeekAgenda };
